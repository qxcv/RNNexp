...
def readCRFgraph(poseDataset, noise=1e-10, forecast_on_noisy_features=False):
    ...
    global nodeNames, nodeList, nodeToEdgeConnections, nodeConnections, \
        nodeFeatureLength, edgeList, edgeFeatures
    ...
    nodeFeatureLength = {}
    for node_name, node_type in zip(lines[0].strip().split(','),
                                    lines[1].strip().split(',')):
    ...
    for nodeName in nodeNames.keys():
        ...
        validate_nodeRNNFeatures = copy.deepcopy(
            validate_edge_features[edgeType])
        forecast_nodeRNNFeatures = copy.deepcopy(
            forecast_edge_features[edgeType])
        ...
        for edgeType in edgeList:
            ...
            validate_nodeRNNFeatures = np.concatenate(
                (validate_nodeRNNFeatures, validate_edge_features[edgeType]),
                axis=2)
            forecast_nodeRNNFeatures = np.concatenate(
    ...
    return nodeNames, nodeList, nodeFeatureLength, nodeConnections, \

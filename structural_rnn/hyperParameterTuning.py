...
if train_model == 'srnn':
    ...
    params['use_noise'] = 1
    params['noise_schedule'] = [250, 0.5e3, 1e3, 1.3e3, 2e3, 2.5e3,
                                3.3e3]  # Add noise after these many iterations
    params['noise_rate_schedule'] = [0.01, 0.05, 0.1, 0.2, 0.3, 0.5,
                                     0.7]  # Variance of noise to add
    ...
    params['batch_size'] = 100
    params[
        'lstm_size'] = 1000  # This paprameter is same as the one used by Fragkiadaki et al. ICCV'15
    params[
        'fc_size'] = 500  # This paprameter is same as the one used by Fragkiadaki et al. ICCV'15
    params['use_noise'] = 1
    params['noise_schedule'] = [250, 0.5e3, 1e3, 1.3e3, 2e3, 2.5e3,
                                3.3e3]  # Add noise after these many iterations
    params['noise_rate_schedule'] = [0.01, 0.05, 0.1, 0.2, 0.3, 0.5,
...
if len(gpus) > 0:
    if use_gpu >= len(gpus):
        use_gpu = 0
    my_env[
        'THEANO_FLAGS'] = 'mode=FAST_RUN,device=gpu{0},floatX=float32'.format(
            gpus[use_gpu])
    use_gpu += 1
else:
    my_env['THEANO_FLAGS'] = 'mode=FAST_RUN,device=cpu,floatX=float32'.format(
        use_gpu)

# Setting directory to dump trained models and then executing trainDRA.py

params[
    'checkpoint_path'] = 'checkpoints_{0}_T_{2}_bs_{1}_tg_{3}_ls_{4}_fc_{5}_demo'.format(
